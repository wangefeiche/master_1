...
class Algorithm:
     ...
     def run(self, time, S_time_interval, S_send_data_size, S_chunk_len, S_rebuf, S_buffer_size, S_play_time_len,S_end_delay, S_decision_flag, S_buffer_flag,S_cdn_flag, end_of_video, cdn_newest_id,download_id,cdn_has_frame, IntialVars):
            ...
            bit_rate = 0
            flag = 0 
            state_write_file = "state_write_file.txt"
            result_write_file = "result_write_file.txt"
            f = open(state_write_file,'a+')
            r = open(result_write_file,'a+')
            for i in range(7000):
            ...
            for j in range(segment_count):
                ...
                f.writelines(str([time, S_time_interval[-1-j], S_send_data_size[-1-j], S_chunk_len[-1-j], S_rebuf[-1-j], S_buffer_size[-1-j], S_play_time_len[-1-j],S_end_delay[-1-j], S_decision_flag[-1-j], S_buffer_flag[-1-j],S_cdn_flag[-1-j]])) 
            ...
            #     target_buffer = 1
            # target_buffer = 0
            r.writelines(str([bit_rate,target_buffer]))
            f.close()
            r.close()
